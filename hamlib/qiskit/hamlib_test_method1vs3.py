...
def construct_circuits(num_qubits):
    ...
    circuits['Method 1 K = 10'] = HamiltonianSimulation(num_qubits, K=10, t=1,
                                                 hamiltonian="TFIM", init_state="checkerboard",
                                                 w=1, hx=np.ones(100), hz=np.ones(100),
                                                 use_XX_YY_ZZ_gates=False, method=1)

    circuits['Method 1 K = 5'] = HamiltonianSimulation(num_qubits, K=5, t=1,
                                                 hamiltonian="TFIM", init_state="checkerboard",
                                                 w=1, hx=np.ones(100), hz=np.ones(100),
                                                 use_XX_YY_ZZ_gates=False, method=1)

    circuits['Method 1 K = 10 Inverse'] = circuits['Method 1 K = 10'].remove_final_measurements(False).inverse().measure_all(inplace=False)

    circuits['Method 1 K = 5 Inverse'] = circuits['Method 1 K = 5'].remove_final_measurements(False).inverse().measure_all(inplace=False) 

    circuits['Method 1 K = 10 t=1E-9'] = HamiltonianSimulation(num_qubits, K=10, t=1E-9,
                                                 hamiltonian="TFIM", init_state="checkerboard",
                                                 w=1, hx=np.ones(100), hz=np.ones(100),
                                                 use_XX_YY_ZZ_gates=False, method=1)

    circuits['Method 3 K = 5'] = HamiltonianSimulation(num_qubits, K=5, t=1,
                                                 hamiltonian="TFIM", init_state="checkerboard",
                                                 w=1, hx=np.ones(100), hz=np.ones(100),
                                                 use_XX_YY_ZZ_gates=False, method=3)
...
def construct_hamsim_circuits(num_qubits):
    ...
    circuits['Method 1 K = 10'] = TfimHamiltonianKernel(num_qubits, K=10, t=1,
                                                 hamiltonian="TFIM", init_state="checkerboard",
                                                 w=1, hx=np.ones(100), hz=np.ones(100),
                                                 use_XX_YY_ZZ_gates=False, method=1, random_pauli_flag = False).overall_circuit()

    circuits['Method 1 K = 5'] = TfimHamiltonianKernel(num_qubits, K=5, t=1,
                                                 hamiltonian="TFIM", init_state="checkerboard",
                                                 w=1, hx=np.ones(100), hz=np.ones(100),
                                                 use_XX_YY_ZZ_gates=False, method=1, random_pauli_flag = False).overall_circuit()
    ...
    circuits['Method 1 K = 10 t=1E-9'] = TfimHamiltonianKernel(num_qubits, K=10, t=1E-9,
                                                 hamiltonian="TFIM", init_state="checkerboard",
                                                 w=1, hx=np.ones(100), hz=np.ones(100),
                                                 use_XX_YY_ZZ_gates=False, method=1, random_pauli_flag = False).overall_circuit()

    circuits['Method 3 K = 5'] = TfimHamiltonianKernel(num_qubits, K=5, t=1,
                                                 hamiltonian="TFIM", init_state="checkerboard",
                                                 w=1, hx=np.ones(100), hz=np.ones(100),
                                                 use_XX_YY_ZZ_gates=False, method=3, random_pauli_flag = False).overall_circuit()
...
def get_circuit_metrics(transpiled_qc):
    qc_depth, qc_size, qc_count_ops, qc_xi, qc_n2q = ex.get_circuit_metrics(transpiled_qc)
    return {
        "depth": qc_depth,
        "size": qc_size,
...
if __name__ == "__main__":
    ...
    for num_qubits in tqdm(valid_qubits):
        ...
        for method, circuit in tqdm(circuits.items(), leave=False):
            ...
            noisy_counts, circuit_metrics = run_experiment(circuit, num_shots, noise_model)
